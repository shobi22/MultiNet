...
def evaluate(hypes, sess, image_pl, inf_out):
    #add the model needs to be classified
	if hypes["only_road"]:
        model_list = ['road']
    else:
        model_list = ['road', 'cross']

	#evaluate the validation and training	
	...
	#evaluation 
	#find the accuracy, mean accuracy, precision and recall
...
def resize_input(hypes, image):
    ...
    jitter = hypes['jitter']
	#accoriding to the options in the jitter perform the resizing
...
def evaluate_data(hypes, sess, image_pl, inf_out, validation=True):
    ...
    #if its validation, add the validation file to data file
