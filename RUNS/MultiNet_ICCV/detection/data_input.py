...
#function to load the kitti data set
def _load_kitti_txt(kitti_txt, hypes, jitter=False, random_shuffel=True):
    ...
    for epoch in itertools.count():
        ...
        for file in files:
            ...
            if im.shape[2] == 4:
                im = im[:, :, :3]
			#if the images are not the height and widhth specified in hypes, then rescale the image
...
def create_queues(hypes, phase):
	...
	#grid size specified as in hypes
...
#checks whether the input data is for validation or training purpose
def inputs(hypes, q, phase):
	#if its for validation
    if phase == 'val':
        image, confidences, boxes, mask = q.dequeue() #enqueuing
        image = tf.expand_dims(image, 0) #expand the dimension of the image
